...
namespace AppForDoctorWebServices.Controllers
{
    public class PatientsController : ApiController
    {
        ...
        public IHttpActionResult PostPatient(Patient patient)
        {
            ...
            return CreatedAtRoute("DefaultApi", new { id = patient.PersonneId }, patient);
        }
        ...
        {
            return db.Patients.Count(e => e.PersonneId == id) > 0;
        }

        [Route("Nom/{nom}/Patients")]
        [HttpGet]
        public IEnumerable<Patient> FindImmunisationByCustomer(string nom)

        {
            var query = from i in db.Patients
                        where i.Nom.StartsWith(nom)
                        select i;
            return query.ToList();
        }
    }
